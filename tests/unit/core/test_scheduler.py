...
from pantainos.core.scheduler import ScheduleManager
...
async def test_interval_execution(schedule_manager, mock_event_bus):
    ...
    assert call_args[0] == "@interval"  # event type
    assert "seconds" in call_args[1]  # event data should contain interval info
...
async def test_execution_count_increments(schedule_manager, mock_event_bus):
    ...
    first_call_data = mock_event_bus.emit.call_args_list[0][0][1]
    last_call_data = mock_event_bus.emit.call_args_list[-1][0][1]
    assert last_call_data["execution_count"] > first_call_data["execution_count"]
